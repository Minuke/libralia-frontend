...
export const matchingPasswordsValidator: ValidatorFn = (group: AbstractControl): ValidationErrors | null => {
	const password1Control = group.get("password1");
	const password2Control = group.get("password2");

	if (!password1Control || !password2Control) return null;

	// Si las contraseñas no coinciden, asigna el error en cada control
	if (password1Control.value !== password2Control.value) {
		...
		// Si coinciden, elimina el error de 'passwordMismatch' sin borrar otros errores posibles
